...
; ********************************************************
; CS 201 HW #3  DUE Wednesday, 10/8/2025 at 11:59 pm
;                via gradescope
; ****************************************************************
...
; ============ DESCRIPTION OF HOW MY TURING MACHINE WORKS ================================
; My Turing machine works by marking the start of a “window” into the tape with
; a marker ‘A. Then, depending on whether the first element of the window is 
; a 0 or a 1, the Turing machine branches to place either the literal ‘ZERO 
; for 0 or ‘ONE for 1 at the end of the tape after walking the whole thing 
; (it knows this by either encountering ‘b, or ‘ZERO/‘ONE from the previous 
; iteration). Each branch knows the value at the end of the window, and walks back 
; to ‘A, replacing ‘A, then moving to the right. The benefit of using ‘ZERO/‘ONE as an 
; intermediate is that it marks the end of the window of numbers that need to be 
; reversed, capping us running away computationally on the tape while also allowing for 
; an easy swap to their numerical representation once the end of the window is reached. 
; I clean up in ‘q10 by moving back to the start of the tape (because my algorithm ends 
; in the middle of the binary string) and in ‘q11 I handle an edge case I noticed in 
; the test suite (one-element tape)
...
(define tm-reverse
  (list
    ...
    (ins 'q3 1 'q4 'ZERO 'L) ; q4 rewinds to A and inserts 1
    (ins 'q3 0 'q5 'ZERO 'L) ; q5 rewinds to A and inserts 0
    (ins 'q3 'A 'q11 0 'L) ; handle edge case for 1 digit string
...
;-----
    ...
    (ins 'q7 1 'q8 'ONE 'L) ; q8 rewinds to A and inserts 1
    (ins 'q7 0 'q9 'ONE 'L) ; q9 rewinds to A and inserts 0
    (ins 'q7 'A 'q11 1 'L) ; handle edge case for 1 digit string
...
;---
    ...
    (ins 'q11 'b 'exit 'b 'R) ; handles edge case with one char in string
  ))
...
; This Turing machine was the easiest of the three to pull off. Essentially, this Turing 
; machine keeps track of whether the number of 1’s so far is even or odd by branching (‘q3 and ‘q6). 
; Every new digit eaten by the machine saves its location on the tape as ‘ret and walks backward to 
; where the parity is stored, remembering it, and then it walks back to the position being processed 
; to replace ‘ret with the parity. It’s okay to erase the tape as we work on it because we only need 
; to know the current working tape entry as well as the parity to calculate a new parity. This Turing 
; machine naturally halts.
...
; ============= OVERVIEW DESCRIPTION OF HOW IT WORKS ==================================
; My Turing machine works by marking the first 1 it sees, because every 1 acts 
; as the door between the zeroes and ones. Then, it moves along the tape 
; looking for the first zero anywhere in the binary string after the door. If we find a
; zero, we swap its value with a one and walk back to the door (marked ‘ONE) 
; where we ultimately replace it with a zero. The process then repeats, trying 
; to find more “doors” and then swapping them with zeroes. I clean up the tape 
; in ‘q5 by replacing the last  ‘ONE with its numerical correspondent and moving 
...
(define tm-sort
  (list
    ...
    ; (ins 'q1 0 'q2 0 'R)
    ; ; (ins 'q1 1 'q3 0 'R)
    ; ; q2 is a pure zero straight prefix
    ; ; ; (ins 'q2 0 'q2 0 'R) ;lovely
    ; (ins 'q2 1 'q3 1 'R) ; uh oh! our special case is broken.
    ; (ins 'q3 1 'q3 1 'R) ;lovely
    ; (ins 'q3 0 'q4 1 'L) ;uh oh, broke chain of 1s.
    ; (ins 'q4 1 'q2 0 'R)
    ; (ins 'q2 0 'q2 0 'R)
    ; (ins 'q2 1 'q3 1 'R)
