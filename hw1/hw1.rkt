...
(define (bin2dec num)
  (if (empty? num) 
    0
    (+ (* (expt 2 (sub1 (length num))) (car num)) (bin2dec (cdr num)))
  ))

(define (dec2bin n)
  (if (zero? n) '() (append
  (dec2bin (exact-floor (/ n 2))) (list (remainder n 2)))))
  ...
  (if (< n 10) 
    ...
    (if (< n 16)
      (cdr 
        (assq n dec2hexlist))
      (append
        (dec2hex (exact-floor (/ n 16)))
        (list 
          (if (< (remainder n 16) 10) 
            (remainder n 16)
            (cadr 
              (assq (remainder n 16) dec2hexlist))
      )))))))
...
(define (myassq value alist)
  (if (empty? alist)
    empty
    (if (eq? (caar alist) value) (car alist) (myassq value (cdr alist)))
))
...
(define (inflate lst [value 1])
  (map (lambda (n) (if (number? n) (+ n value) n)) lst))
...
(define (iterate start proc n) 
  (if (zero? n) '() (let ([result (proc start)]) (cons result (iterate result proc (- n 1))))))
...
(define (compound start proc test)
  (if (test start) '() (let ([now (proc start)]) (cons now (compound now proc test)))))
...
(define (power-set lst)
  (if (empty? lst) 
    ...
    (append 
      (map 
        (lambda (n) (cons (car lst) n))
        (power-set (cdr lst)))
      (power-set (cdr lst)))
  ))

; ## THIS IS THE GRAVEYARD OF SNIPPETS FROM MY ATTEMPTS THAT DIDN'T WORK
    ; (let ([x (power-set (cdr lst) (+ 1 depth))])
    ; (displayln (format "~a~a and ~a: ~a" (string-multiply "  " depth) (car lst) (cdr lst) (cons (car lst) x) ))
    ; (cons (cons x '()) (cons (car lst) x ))))
;   (let* ([l (reverse lst)] [x (power-set (cdr l) (+ depth 1))])
;   (displayln (format "~a(~a and ~a)" (string-multiply "  " depth) (car l) x))
;   (map (lambda (data) (begin 
;     (displayln (cons (car l) data))
;     (cons (car l) data)
;   )) x)
;   )
;   (power-set (cdr lst) (+ depth 1))
;   )
;   ))
...
(define (all-factors n)
  (sort (remove-duplicates (map (lambda (primes) (apply * primes)) (power-set (prime-factors n)))) <))
